...
function ExpertiseCard({ c }: { c: Card }) {
  ...
  className="
    ...
    before:rounded-[var(--rule-radius,9999px)]
    before:bg-[var(--c-lilac,#a78bfa)]
  "
  style={{
    // tweak these once; remove or change per card if you want
    ['--rule-w' as any]: '4px',        // thickness
    ['--rule-gap' as any]: '36px',     // gap between rule & text
    ['--rule-radius' as any]: '0px' // 0px=square, 8px=soft, 9999px=pill
  }}
>
  {c.blurb}
</p>
